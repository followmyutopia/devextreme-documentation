...
The [TreeView](/Documentation/ApiReference/UI_Widgets/dxTreeView/) widget displays specified data using a tree structure.
...
The widget requires each data item to have a key field whose value is unique within the entire array. By default, the widget uses the **id** field as a key. If you need to use another field as a key, pass the required field name to the [keyExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#keyExpr) option.
...
- **key**  
 The key value of the current node. This field holds a value obtained from the [data source](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#dataSource) item associated with the current node according to the [keyExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#keyExpr) option value. If the **keyExpr** value is not specified, the **key** field holds the auto generated key value.
...
- **text**  
 The text displayed by the node. This field holds a value obtained from the [data source](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#dataSource) item associated with the current node according to the [displayExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#displayExpr) option value.
...
The widget also enables you to get the list of all nodes using the [getNodes()](/Documentation/ApiReference/UI_Widgets/dxTreeView/Methods/#getNodes) method.
...
You can assign the required field name to the [itemsExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#itemsExpr) option to specify another field used to hold nested items.
...
The [TreeView](/Documentation/ApiReference/UI_Widgets/dxTreeView/) widget can also use an array that has a plain structure. Assign 'plane' to the [dataStructure](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#dataStructure) option to use a plain array as the data source. In this case, each item should contain the **parentId** field, which holds the key value of the parent item.
...
You can use the [parentIdExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#parentIdExpr) option to specify another field for holding the parent key.
...
If the data source associated with the widget has a plain structure, you can enable the Virtual mode, which can significantly increase performance when working with a large data source. If Virtual mode is enabled, the widget initially loads only root items, while child items are loaded when their parent is being expanded. To enable Virtual mode, set the [virtualModeEnabled](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#virtualModeEnabled) option to *true*.

If Virtual mode is enabled, the widget requires you to specify the [parentId](/Documentation/ApiReference/UI_Widgets/dxTreeView/Default_Item_Template/#parentId) value of root items using the [rootValue](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#rootValue) option. The default value of the option is 0.
...
[note]Virtual mode also requires the widget to access data using a [DataSource](/Documentation/ApiReference/Data_Layer/DataSource/) instance able to [filter data](/Documentation/Guide/Data_Layer/Data_Layer/#Reading_Data/Filtering) by the **parentId** value.
...
You can assign the required field name to the [expandedExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#expandedExpr) option to specify another field used to determine the expansion state of an item.
...
You can also expand and collapse items at runtime using the [expandItem(itemElement)](/Documentation/ApiReference/UI_Widgets/dxTreeView/Methods/#expandItemitemElement) and [collapseItem(itemElement)](/Documentation/ApiReference/UI_Widgets/dxTreeView/Methods/#collapseItemitemElement) methods respectively.

**Select widget items**

If the [showCheckBoxes](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#showCheckBoxes) option is set to *true*, the widget displays a check box for each tree item. In this case, the **selected** field of a data item specifies whether the item is displayed selected or not.
...
To specify another field determining the selection state of an item, use the [selectedExpr](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#selectedExpr) option.
...
You can assign *true* to the [selectAllEnabled](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#selectAllEnabled) option to display a check box that selects or unselects all tree items. The check box text is specified by the [selectAllText](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#selectAllText) option.
...
The **TreeView** widget includes the [searchValue](/Documentation/ApiReference/UI_Widgets/dxTreeView/Configuration/#searchValue) option, which enables you to filter its items. If this option holds a non-empty string, the widget displays only items containing the specified string and their parents.

To implement a search box for the tree view, create the [TextBox](/Documentation/ApiReference/UI_Widgets/dxTextBox/) widget and pass its value to the **searchValue** option of the tree view each time the text box value has changed.
