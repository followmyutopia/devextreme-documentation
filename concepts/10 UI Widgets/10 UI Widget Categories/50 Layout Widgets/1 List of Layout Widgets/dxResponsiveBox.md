...
The [ResponsiveBox](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/) widget enables you to create a site with different layouts for different devices as illustrated by the following image.

![Different Layouts](/Content/images/doc/16_1/UiWidgets/responsive-box.png)
...
To specify custom screen factors, use the [screenByWidth](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/Configuration/#screenByWidth) option of the widget. The function passed to this option takes on the screen width as a parameter and returns a string containing the screen factor corresponding to the current screen width.
...
The layout elements are positioned against the widget grid specified using the [cols](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/Configuration/#cols) and [rows](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/Configuration/#rows) options.
...
The widget elements are usually specified using the **dxItem** markup component, as described in the [Specify Layout Elements](/Documentation/Guide/UI_Widgets/UI_Widget_Categories/Layout_Widgets/#Specify_Layout_Elements) section of this article. The **ResponsiveBox** items support the following options used to specify the element position, size and screen factor.
...
For information on the complete set of item options, refer to the [Default Item Template](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/Default_Item_Template/) reference section of this widget.
...
[note]If you display [data visualization widgets](/Documentation/ApiReference/Data_Visualization_Widgets/) within the [ResponsiveBox](/Documentation/ApiReference/UI_Widgets/dxResponsiveBox/) widget, **ResponsiveBox** automatically locates them, however, it cannot resize them automatically. Thus, to display a data visualization widget within **ResponsiveBox**, you should specify its width and height in pixels. If the data visualization widget size is not specified or specified relatively to the parent container ('100%', 'auto', 'inherit'), the **ResponsiveBox** size is calculated incorrectly.
