...
The [SlideOutView](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/) widget allows you to slide-out the current view to reveal a custom menu.

This widget is similar to the [SlideOut](/Documentation/Guide/UI_Widgets/UI_Widget_Categories/Collection_Container_Widgets/#List_of_Collection_Container_Widgets/dxSlideOut) widget. In its simplest form, it consists of a panel that slides out to reveal a menu. However, unlike the **SlideOut** widget, **SlideOutView** does not hold items. It contains only content and menu panels, which enable you to use any collection widget as a menu.

**Specify the widget content**

Implement a function that renders the widget content and pass it to the [contentTemplate](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#contentTemplate) option.
...
In **MVVM approach**, create a content template and pass its name or element to the [contentTemplate](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#contentTemplate) option.
...
The **SlideOutView** widget can use any collection widget as a menu. Implement a function that renders the menu panel and pass it to the [menuTemplate](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuTemplate) option.
...
In **MVVM Approach**, create a content template and pass its name or element to the [menuTemplate](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuTemplate) option.
...
To show or hide the menu, call the [showMenu()](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Methods/#showMenu) or [hideMenu()](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Methods/#hideMenu) methods respectively. To access a widget method, obtain the widget instance first.
...
Also you can use the [menuVisible](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuVisible) option to show or hide the menu depending on the variable value. In **jQuery approach**, set the option value using the [option(optionName, optionValue)](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Methods/#optionoptionName_optionValue) method. In **MVVM approach**, provide two-way binding between the configuration option and the corresponding field of a view or application model.
...
To specify the current menu position use the [menuPosition](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuPosition) option. This option accepts two values: *normal* and *inverted*. By default, [menuPosition](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuPosition) is set to *normal*. Normally, the menu is located at the left side of the screen. To display the menu at the opposite side assign 'inverted' to this option.

If you need to switch the display of the widget to right-to-left, enable the [rtlEnabled](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#rtlEnabled) option. When the option value is *true*, the text flows from right to left, and the layout the component's elements is reversed. Note, in a right-to-left representation, normal position of the menu is the right side of the screen. Assign 'inverted' to the [menuPosition](/Documentation/ApiReference/UI_Widgets/dxSlideOutView/Configuration/#menuPosition) option to show the menu on the left side.
