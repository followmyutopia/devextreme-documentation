...
#####See Also#####
- [Reference](/Documentation/ApiReference/UI_Widgets/) | **WidgetName** | **Configuration** - describes all options of a specific DevExtreme widget.

You can initialize widget options with the value of a [scope](http://docs.angularjs.org/guide/scope) property. For example, the following code declares the `menuItems` property within the scope of a controller. The `dataSource` option of a **Menu** is initialized with the value of this property.
...
[note]Initializing widget options in this manner does not mean that the widget option will be changed once its scope property is changed. If you are looking for this kind of data binding, refer to the [Change Options](/Documentation/Guide/UI_Widgets/Basics/Widget_Basics_-_AngularJS/#Change_Options) topic.
