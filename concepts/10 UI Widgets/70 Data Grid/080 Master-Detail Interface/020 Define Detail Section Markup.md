...
Once loaded, the detail section's content remains cached until the grid's [page](/Documentation/Guide/UI_Widgets/Data_Grid/Data_Navigation/#Pager_Navigation) switches or web page reloads. It prevents data from loading from the [data source](/Documentation/Guide/UI_Widgets/Data_Grid/Data_Binding/) each time a user expands the detail section. Cashing increases the loading speed of detail sections expanded multiple times and reduces the server load if you use the [remote data source](/Documentation/Guide/UI_Widgets/Data_Grid/Data_Binding/#Provide_Data/Using_the_Data_Layer/Using_an_ODataStore).
