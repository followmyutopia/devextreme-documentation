...
The widget searches the required items by the value of the property specified using the [displayExpr](/Documentation/ApiReference/UI_Widgets/dxLookup/Configuration/#displayExpr) option. For example, if the **displayExpr** option is set to "name", the widget displays items whose **name** property value satisfies the search criterion.

The widget starts searching after the specified timeout that occurs after an end-user has modified the search box value has expired, provided that the length of the string typed in the search field is more than or equal to the minimum length specified using the [minSearchLength](/Documentation/ApiReference/UI_Widgets/dxLookup/Configuration/#minSearchLength) option. You can specify the timeout using the [searchTimeout](/Documentation/ApiReference/UI_Widgets/dxLookup/Configuration/#searchTimeout) option.
