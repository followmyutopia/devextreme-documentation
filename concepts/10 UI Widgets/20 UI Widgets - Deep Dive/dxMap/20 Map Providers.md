...
The widget supports [Bing](http://www.microsoft.com/maps) and [Google](https://developers.google.com/maps) map providers to display the map. Use the [provider](/Documentation/ApiReference/UI_Widgets/dxMap/Configuration/#provider) option to specify the required map provider. You can assign one of the following values to the **provider** option.
