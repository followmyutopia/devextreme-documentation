...
All the modifications are saved to the [bound data source](/Documentation/Guide/UI_Widgets/Scheduler/Appointments/#Provide_a_Data_Source).
...
By default, if you are modifying a recurring appointment using a UI, the widget displays a dialog that prompts you to edit only the active appointment or the entire series. To change this behavior, assign the required value to the [recurrenceEditMode](http://js.devexpress.com/Documentation/ApiReference/UI_Widgets/dxScheduler/Configuration/#recurrenceEditMode) option. The option accepts the following values.
