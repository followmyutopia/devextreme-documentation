The easiest way to implement a custom palette is to assign an array of colors to the **palette** option (see [Apply a Palette](/Documentation/Guide/Themes_and_Styles/SVG-Based_Widgets_Customization/#Palettes/Apply_a_Palette)). However, this approach is only useful for a single widget or several widgets of the same type.
...
The custom palette should be registered using the [registerPalette(paletteName, palette)](/Documentation/ApiReference/Common/utils/viz/Methods/#registerPalettepaletteName_palette) method:
...
To apply it, assign the name used in the registration to the widgets' **palette** options as shown in the [Apply a Palette](/Documentation/Guide/Themes_and_Styles/SVG-Based_Widgets_Customization/#Palettes/Apply_a_Palette) article.
