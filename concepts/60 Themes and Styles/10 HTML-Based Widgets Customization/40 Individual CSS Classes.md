...
[note] The internal CSS structure may be changed between releases without notice. Taking that into account, we recommend customizing widgets [using their API](/Documentation/Guide/Themes_and_Styles/HTML-Based_Widgets_Customization/#Widgets_API) instead of CSS if it is possible because you will be notified if the API is changed.
