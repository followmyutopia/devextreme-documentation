Templates allow you to customize widget elements. In the following code, an [itemTemplate](/Documentation/ApiReference/UI_Widgets/dxList/Configuration/#itemTemplate) called `listItem` and a [groupTemplate](/Documentation/ApiReference/UI_Widgets/dxList/Configuration/#groupTemplate) called `listGroup` customize items and groups in the [List](/Demos/WidgetsGallery/Demo/List/GroupedList/Angular/Light/) widget. Inside the templates, the `itemData` and `groupData` variables expose item and group data objects; the `itemIndex` variable gives access to the item index.
