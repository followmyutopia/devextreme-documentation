If you develop using the [Node.js](http://nodejs.org/) platform, utilize a special script allowing you to parse binary sources and generate **VectorMap**-compatible files from them. This script can be found in the **Lib/js/vectormap-utils** folder of your [DevExtreme package](/Documentation/Guide/Common/DevExtreme_Packages/). Load this script as a [module](http://nodejs.org/api/modules.html) using the following code.
...
- **Using Data from a *Buffer* Source**		
	The following code sample demonstrates how to use the **parse()** method for parsing data from any source of the *[Buffer](http://nodejs.org/api/buffer.html)* type.		
...
Specifies the format of the resulting file. When this option is set to *true*, the resulting file has the *'.json'* extension and can be [used in VectorMap as a JSON object](/Documentation/Guide/Data_Visualization/VectorMap/Providing_Data/#VectorMap_Providing_Data_Data_for_Areas_Using_a_JSON_object). When this option is set to *false*, the resulting file has the *'.js'* extension and can be [used in VectorMap as a script](/Documentation/Guide/Data_Visualization/VectorMap/Providing_Data/#VectorMap_Providing_Data_Data_for_Areas_Using_a_Script).
