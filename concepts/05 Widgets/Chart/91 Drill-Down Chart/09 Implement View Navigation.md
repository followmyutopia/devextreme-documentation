To navigate from the first to the second view, filter data by a different `parentID` in the **Chart**'s [onPointClick](/Documentation/ApiReference/Data_Visualization_Widgets/dxChart/Configuration/#onPointClick) event handler. To navigate back, add the [Button](/Documentation/ApiReference/UI_Widgets/dxButton/) widget and reset the filter in the [onClick](/Documentation/ApiReference/UI_Widgets/dxButton/Configuration/#onClick) event handler. Distinguish between levels using the `isFirstLevel` flag.
