...
To associate the **Button** widget with the created element, the `dxButton` constructor must be called once the document has been loaded. So, add a JavaScript file to your project to provide code behind the view. Link this file within the view.
...
Add the **CheckBox** widget in the same manner as you added the **Button** widget. To handle check box value changes, implement the appropriate function and assign it to the [onValueChanged](/Documentation/ApiReference/UI_Widgets/dxCheckBox/Configuration/#onValueChanged) option of the **CheckBox** widget. In this function, access the button widget by calling the `dxButton` constructor of the button element and by passing "instance" as a parameter. To change a configuration option of the accessed button, call the [option](/Documentation/ApiReference/UI_Widgets/dxCheckBox/Methods/#option) method passing the following arguments.
