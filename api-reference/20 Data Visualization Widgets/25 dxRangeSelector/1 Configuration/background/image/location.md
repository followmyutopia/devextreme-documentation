...
When configuring the widget using [ASP.NET MVC Wrappers](/Documentation/Guide/ASP.NET_MVC_Wrappers/Fundamentals/), specify this option using the `BackgroundImageLocation` enum. This enum accepts the same values, but they start with an upper-case letter, for example, *'leftTop'* becomes `LeftTop`.
<!--/fullDescription-->