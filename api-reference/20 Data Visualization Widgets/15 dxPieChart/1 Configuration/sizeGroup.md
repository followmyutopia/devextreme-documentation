<!--**
/*-------------------------------------------
    Auto-generated file. Do not modify.
-------------------------------------------

**-->
===========================================================================
<!--default-->undefined<!--/default-->
<!--type-->string<!--/type-->
===========================================================================

<!--shortDescription-->
Allows you to display several adjoining pies in the same size.
<!--/shortDescription-->

<!--fullDescription-->
Besides the pie itself, the **PieChart** widget comprises other diverse elements that affect the size of the pie. Therefore, when you display several **PieChart** widgets side by side, their pies may differ in size. To eliminate these differences, collect all the **PieChart** widgets in a single size group by setting their **sizeGroup** option to identical values. The widgets should have identical layouts, that is, the same container's size, position of the [title](/Documentation/Guide/Widgets/PieChart/Visual_Elements/#Chart_Title) and [legend](/Documentation/Guide/Widgets/PieChart/Visual_Elements/#Legend), etc. Note also that a single page can contain many size groups, but a widget can be a member of only one of them.

#####See Also#####
- [size](/Documentation/ApiReference/Data_Visualization_Widgets/dxPieChart/Configuration/size/)
- [diameter](/Documentation/ApiReference/Data_Visualization_Widgets/dxPieChart/Configuration/#diameter)
- [minDiameter](/Documentation/ApiReference/Data_Visualization_Widgets/dxPieChart/Configuration/#minDiameter)
<!--/fullDescription-->