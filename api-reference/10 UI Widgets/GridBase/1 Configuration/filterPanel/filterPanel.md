...
A click on the filter expression opens the [filter builder]({basewidgetpath}/Configuration/#filterBuilder).
...
If you change the filter expression in the filter panel or filter builder, the changes will be reflected in the [filter row](/Documentation/Guide/Widgets/{WidgetName}/Filtering_and_Searching/#Filter_Row) and [header filter](/Documentation/Guide/Widgets/{WidgetName}/Filtering_and_Searching/#Header_Filter), and vice versa. You can disable the synchronization by setting the [filterSyncEnabled]({basewidgetpath}/Configuration/#filterSyncEnabled) option to **false**. In this case, the filter panel remains synchronized only with the filter builder.
