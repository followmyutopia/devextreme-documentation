---
id: dxDateBox.Options.useMaskBehavior
type: Boolean
default: false
---
---
##### shortDescription
Specifies whether to control user input using a mask created based on the [displayFormat](/api-reference/10%20UI%20Widgets/dxDateBox/1%20Configuration/displayFormat.md '/Documentation/ApiReference/UI_Widgets/dxDateBox/Configuration/#displayFormat').

---
#include common-demobutton with {
    url: "/Demos/WidgetsGallery/Demo/DateBox/Formatting/"
}

#####See Also#####
- [Value Formatting and Masked Input](/concepts/05%20Widgets/DateBox/04%20Value%20Formatting%20and%20Masked%20Input.md '/Documentation/Guide/Widgets/DateBox/Value_Formatting_and_Masked_Input/')