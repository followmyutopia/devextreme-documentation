...
slideOutData = new DevExpress.data.DataSource({
    store: contacts,
    paginate: false
});
slideOutOptions = {
...
$(function () {
    ...
    showGroups.subscribe(function (grouped) {
        var slideOut = $("#slideOut").dxSlideOut("instance");
        slideOut.beginUpdate();
        if (grouped)
            slideOutData.group(function (dataItem) {
                return dataItem.category;
            });
        else
            slideOutData.group(null);
        slideOut.option("menuGrouped", grouped);
        slideOutData.load().done(function(){
            slideOut.endUpdate();
        });
    });
    slideOutData.load().done(function () {
        $("#slideOut").dxSlideOut("instance").option("selectedIndex", 0);
    });
});